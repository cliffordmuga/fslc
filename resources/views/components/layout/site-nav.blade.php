{{-- Fixed public navigation — search, desktop + mobile menus --}}
@php
    $navItems = [
...
<nav x-data="{ open: false, search: false, scrolled: false }"
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex items-center justify-between h-14 lg:h-16">
            <a href="{{ route('home') }}" class="flex items-center gap-2.5 group" aria-label="{{ config('app.name', env('APP_NAME', 'App')) }}">
            ...
            <div class="hidden lg:flex items-center gap-2">
                ...
                @auth
                    <a href="{{ route('dashboard') }}" class="text-sm text-neutral-600 hover:text-neutral-900 px-3 py-2 transition-colors">Dashboard</a>
                @else
                    <a href="{{ route('login') }}" class="text-sm text-neutral-600 hover:text-neutral-900 px-3 py-2 transition-colors">Sign In</a>
                    <a href="{{ nav_primary_cta_url() }}" class="btn-primary text-xs px-5 py-2.5">{{ config('forefront.nav_primary_cta.label', 'Request HMIS Demo') }}</a>
                @endauth
            </div>
    ...
    <div x-show="open" x-transition
        ...
        <div class="divide-y divide-neutral-100">
            ...
            <div class="px-5 py-4 flex flex-col gap-2">
                @auth
                    <a href="{{ route('dashboard') }}" class="btn-secondary text-sm py-2.5" @click="open = false">Dashboard</a>
                @else
                    <a href="{{ nav_primary_cta_url() }}" class="btn-primary text-sm text-center py-2.5" @click="open = false">{{ config('forefront.nav_primary_cta.label', 'Request HMIS Demo') }}</a>
                    <a href="{{ route('login') }}" class="btn-secondary text-sm text-center py-2.5" @click="open = false">Sign In</a>
                @endauth
            </div>
        </div>
    </div>
