{{-- resources/views/components/cms/whatsapp-cta.blade.php --}}
...
@if ($protected)
    <span class="protected-contact {{ $classes }} cursor-pointer" data-type="whatsapp"
        data-label="{{ $label }}" data-value="{{ $encodedPhone }}"
        @if ($encodedMessage) data-message="{{ $encodedMessage }}" @endif aria-label="{{ $label }}"
...
@else
    <a class="{{ $classes }}" href="{{ $href }}" target="_blank" rel="noopener noreferrer nofollow"
        aria-label="{{ $label }}">
        @if ($slot->isEmpty())
            {!! $fallbackInner !!}
        @else
