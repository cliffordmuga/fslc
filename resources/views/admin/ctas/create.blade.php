...
@section('title', $cta->exists ? 'Edit CTA' : 'Create CTA')
@section('header', $cta->exists ? 'Edit CTA' : 'Create CTA')
@section('content')
    <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
        <form method="POST" action="{{ $cta->exists ? route('admin.ctas.update', $cta) : route('admin.ctas.store') }}" class="card-base p-6 space-y-5">
            @csrf
            ...
            <div>
                ...
                <input type="text" name="action" value="{{ old('action', $cta->action) }}" required class="input-base w-full" placeholder="/contact?inquiry_type=hmis-demo#contact-form">
            </div>
